...
export default function ExpensesPage() {
  const [expenses, setExpenses] = useState([
    { id: 1, category: 'Office Supplies', amount: 5000, date: '2024-01-15', description: 'Printer paper and ink', status: 'Approved' },
    { id: 2, category: 'Travel', amount: 25000, date: '2024-01-16', description: 'Client meeting travel', status: 'Pending' },
  ]);
  const [showForm, setShowForm] = useState(false);
  const [formData, setFormData] = useState({ category: 'Office Supplies', amount: '', date: '', description: '' });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setExpenses([...expenses, { id: expenses.length + 1, ...formData, amount: Number(formData.amount), status: 'Pending' }]);
    setFormData({ category: 'Office Supplies', amount: '', date: '', description: '' });
    setShowForm(false);
  };
          ...
          <tbody>
            {expenses.map(exp => (
              <tr key={exp.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="py-4 px-6 text-gray-600">₦{exp.amount.toLocaleString()}</td>
                <td className="py-4 px-6 text-gray-600">{exp.date}</td>
                <td className="py-4 px-6">
                  <span className={`inline-flex px-3 py-1 rounded-full text-sm font-medium ${exp.status === 'Approved' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                    {exp.status}
                  </span>
                </td>
