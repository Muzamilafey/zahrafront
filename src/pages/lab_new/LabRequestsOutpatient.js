...
const LabRequestsOutpatient = () => {
  ...
  useEffect(() => {
    const fetchOutpatientRequests = async () => {
      try {
        setLoading(true);
        const response = await axiosInstance.get('/lab/orders?patientStatus=outpatient');
        setOrders(response.data.orders);
      } catch (err) {
        console.error('Failed to fetch outpatient lab requests:', err);
    ...
    fetchOutpatientRequests();
  }, [axiosInstance]);

  if (loading) {
    return <div className="text-center p-8">Loading Outpatient Lab Requests...</div>;
  }
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Lab Requests (Outpatient)</h1>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {orders.length > 0 ? (
              ...
              <tr>
                <td colSpan="6" className="text-center py-4 text-sm text-gray-500">No outpatient lab requests found.</td>
              </tr>
            )}
          </tbody>
        </table>
...
export default LabRequestsOutpatient;