...
const InternalPatientLabVisits = () => {
  ...
  useEffect(() => {
    const fetchInternalVisits = async () => {
      try {
        setLoading(true);
        const response = await axiosInstance.get('/lab/orders?patientStatus=inpatient&status=completed');
        setVisits(response.data.orders);
      } catch (err) {
        console.error('Failed to fetch internal lab visits:', err);
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">Internal Patient Lab Visits (Completed Tests)</h1>
            ...
            {table.getRowModel().rows.length > 0 ? (
              ...
              <tr>
                <td colSpan={columns.length} className="text-center py-4 text-sm text-gray-500">
                  No completed lab visits found for internal patients.
                </td>
              </tr>
            )}
          </tbody>
...
export default InternalPatientLabVisits;