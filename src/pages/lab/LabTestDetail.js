...
const LabTestDetail = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setTest(res.data.labTest || res.data);
        setStatus(res.data.labTest?.status || res.data?.status || '');
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const attachResults = async (e) => {
    ...
    try {
      ...
      console.error(err);
      alert('Failed to attach results');
    }
  };
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      <div className="bg-white border rounded p-4 mb-4">
        <div><strong>Patient:</strong> {test.patient?.user?.name || test.patient?.name || '-'}</div>
        <div><strong>Doctor:</strong> {test.doctor?.user?.name || test.doctor?.name || '-'}</div>
        <div><strong>Test:</strong> {test.testType || test.test || '-'}</div>
