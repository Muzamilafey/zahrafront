...
export default function LabTests() {
  ...
  const fetchTests = useCallback(async () => {
    try {
      ...
      setTests(response.data.catalog || response.data.tests || []);
      setLoading(false);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axiosInstance.post('/lab/catalog', newTest);
      setNewTest({
        name: '',
        description: '',
        normalValue: '',
  ...
  const handleDelete = async (testId) => {
    if (window.confirm('Are you sure you want to delete this test?')) {
      try {
        await axiosInstance.delete(`/lab/catalog/${testId}`);
        fetchTests();
      } catch (error) {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {tests.map((test) => (
              <tr key={test._id}>
                ...
                <td className="px-6 py-4 text-sm text-gray-900">${test.price}</td>
                <td className="px-6 py-4">
                  <button onClick={() => handleDelete(test._id)} className="text-red-600 hover:text-red-900">Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
